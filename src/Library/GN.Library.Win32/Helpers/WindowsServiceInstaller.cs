...
namespace GN.Library.Win32.Helpers
{
	...
	public static class WindowsServiceInstaller
	{
		...
		public static bool IsSCMAvailable(bool allAccess = true)
		{
			IntPtr scm = IntPtr.Zero;
			try
			{
				scm = OpenSCManager(allAccess ? ScmAccessRights.CreateService : ScmAccessRights.Connect);
		...
		public static void Install(string serviceName, string displayName, string fileName, bool start = false)
		{
			IntPtr scm = OpenSCManager(ScmAccessRights.AllAccess);
